...
void PlayerWeapon::fire(World& world, RandomSource& rng)
{
    ...
    using FireFn = void (*)(BulletSystem&, World&, float x, float muzzleY, float rateMul);
    static const FireFn kFire[] = {
        [](BulletSystem& b, World& w, float px, float my, float rm)
        {
            b.spawn(w, BulletOwner::Player, WeaponLevel::Vulcan, {px, my}, {0.f, -780.f}, 1);
            w.player.fireCooldown = 0.12f * rm;
        },
        [](BulletSystem& b, World& w, float px, float my, float rm)
        {
            b.spawn(w, BulletOwner::Player, WeaponLevel::Plasma, {px - 10.f, my}, {-40.f, -820.f}, 2);
            b.spawn(w, BulletOwner::Player, WeaponLevel::Plasma, {px + 10.f, my}, {40.f, -820.f}, 2);
            w.player.fireCooldown = 0.14f * rm;
        },
        [](BulletSystem& b, World& w, float px, float my, float rm)
        {
            b.spawn(w, BulletOwner::Player, WeaponLevel::Proton, {px, my}, {0.f, -900.f}, 3);
            b.spawn(w, BulletOwner::Player, WeaponLevel::Proton, {px - 16.f, my + 6.f}, {-90.f, -860.f}, 2);
            b.spawn(w, BulletOwner::Player, WeaponLevel::Proton, {px + 16.f, my + 6.f}, {90.f, -860.f}, 2);
    ...
    if (w >= 0 && w < 3)
        kFire[w](bullets, world, x, muzzleY, rateMul);
